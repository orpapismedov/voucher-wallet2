...
import { X, Save, Upload, QrCode } from 'lucide-react';
...
interface EditVoucherModalProps {
  ...
  onClose: () => void;
  onSave: (id: string, name: string, amount: number, link: string, code: string, qrImage: string) => void;
}
...
const EditVoucherModal = ({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave(voucher.id, voucherName, parseFloat(amount), link.trim(), code.trim(), qrImage);
    handleClose();
  };
          ...
          <div className="form-group">
            ...
            <div className="qr-upload-container">
              <input
                ...
                style={{ display: 'none' }}
              />
              <label htmlFor="qr-upload-edit" className="btn btn-secondary upload-btn">
                <Upload size={16} />
                העלה תמונה
              </label>
              {qrImage && (
